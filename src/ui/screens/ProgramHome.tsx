...
export function ProgramHome({ team, opponent, result, onNavigate, onPregame }: { team: Team; opponent: Team; result: GameResult | null; onNavigate: (view: MainView) => void; onPregame: () => void }) {
  ...
  const programIndex = Number(team.id.split("-").at(-1) ?? 1);
  const recordWins = 10 + ((programIndex * 7) % 9);
  const recordLosses = 5 + ((programIndex * 3) % 6);
  const ranking = 8 + ((programIndex * 11) % 42);
  ...
  return <div className="screen-stack">
    <section className="program-hero" style={{ borderLeftColor: team.colors[0] }}>
      ...
      <div className="program-hero-bottom"><div><span>RECORD</span><b>{recordWins}–{recordLosses}</b></div><div><span>NATIONAL RANK</span><b>#{ranking}</b></div><div><span>PROGRAM RATING</span><b>{Math.round(team.roster.reduce((sum, player) => sum + player.ratings.overall, 0) / team.roster.length)}</b></div><div><span>HEAD COACH</span><b>{team.coach.name}</b></div></div>
    </section>
