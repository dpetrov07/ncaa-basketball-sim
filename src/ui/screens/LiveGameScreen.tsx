...
export function LiveGameScreen({ state, userTeamId, game, onStateChange, onBoxScore }: { state: GameState; userTeamId: string; game?: ScheduledGame; onStateChange: (state: GameState) => void; onBoxScore: () => void }) {
  ...
  const [tab, setTab] = useState<"play" | "lineups" | "stats" | "coaching">("play");
  ...
  return <div className="screen-stack live-screen"><ScreenHeader eyebrow={complete ? "FINAL WHISTLE" : `DAY ${game?.day ?? "—"} · ${state.stoppage.replaceAll("-", " ").toUpperCase()}`} title="Courtside" subtitle={complete ? "Game complete · season result saved" : `${state.period === 1 ? "First half" : state.period > 2 ? `Overtime ${state.period - 2}` : "Second half"} · ${state.possessionTeamId === state.home.team.id ? state.home.team.shortName : state.away.team.shortName} ball`} action={complete ? <button className="text-button" onClick={onBoxScore}>Box score <ChevronRight size={14} /></button> : <span className="live-dot"><i /> LIVE</span>} />
    <section className="live-score"><div className="live-score-main"><div><TeamMark team={state.home.team} size="sm" /><span>{state.home.team.shortName}</span><strong className={state.home.score >= state.away.score ? "leading" : ""}>{state.home.score}</strong></div><aside><b>{state.clock ? clock(state.clock) : "—"}</b><span>{state.period > 2 ? `OT${state.period - 2}` : state.period === 1 ? "1ST" : "2ND"}</span></aside><div><TeamMark team={state.away.team} size="sm" /><span>{state.away.team.shortName}</span><strong className={state.away.score > state.home.score ? "leading" : ""}>{state.away.score}</strong></div></div><div className="live-game-meta"><span>FOULS {state.home.stats.fouls}–{state.away.stats.fouls}</span><span>TIMEOUTS {state.home.timeoutsRemaining}–{state.away.timeoutsRemaining}</span></div></section>
    <nav className="live-tabs" aria-label="Live game views"><button className={tab === "play" ? "active" : ""} onClick={() => setTab("play")}>Play-by-play</button><button className={tab === "lineups" ? "active" : ""} onClick={() => setTab("lineups")}>Lineups</button><button className={tab === "stats" ? "active" : ""} onClick={() => setTab("stats")}>Stats</button>{!complete && <button className={tab === "coaching" ? "active" : ""} onClick={() => setTab("coaching")}>Coaching</button>}</nav>
    {error && <p className="error-message">{error}</p>}
    {!complete && tab === "coaching" && <div className="live-action-row coaching-actions"><button onClick={openLineup}><UsersRound size={15} /> Substitutions</button><button onClick={openStrategy}><SlidersHorizontal size={15} /> Strategy</button><button disabled={userRuntime.timeoutsRemaining <= 0} onClick={() => run((current) => callTimeout(current, userTeamId))}><Timer size={15} /> Timeout ({userRuntime.timeoutsRemaining})</button></div>}
    ...
    {tab === "play" && <section className="live-grid"><div className="live-main"><div className="section-label"><span>RECENT ACTION</span><span>{state.events.length} events</span></div><div className="live-feed">{state.events.slice(-14).reverse().map((event) => <div className={`live-feed-item ${event.kind}`} key={event.id}><small>{event.clock > 0 ? clock(event.clock) : "—"}</small><span /><p>{event.text}</p></div>)}</div></div></section>}
    {tab === "lineups" && <section className="live-side live-lineups-view"><div><div className="section-label"><span>{userRuntime.team.shortName} ON THE FLOOR</span></div>{renderActive(userRuntime)}</div><div><div className="section-label"><span>{opponentRuntime.team.shortName} ON THE FLOOR</span></div>{renderActive(opponentRuntime)}</div></section>}
    {tab === "stats" && <section className="live-stats-view"><div className="live-stat-heading"><span>{state.home.team.shortName}</span><b>TEAM STATS</b><span>{state.away.team.shortName}</span></div><div className="live-stat-grid"><LiveStat label="Points" home={state.home.stats.points} away={state.away.stats.points} /><LiveStat label="Field goals" home={`${state.home.stats.fgm}-${state.home.stats.fga}`} away={`${state.away.stats.fgm}-${state.away.stats.fga}`} /><LiveStat label="Three-pointers" home={`${state.home.stats.threePm}-${state.home.stats.threePa}`} away={`${state.away.stats.threePm}-${state.away.stats.threePa}`} /><LiveStat label="Free throws" home={`${state.home.stats.ftm}-${state.home.stats.fta}`} away={`${state.away.stats.ftm}-${state.away.stats.fta}`} /><LiveStat label="Rebounds" home={state.home.stats.offensiveRebounds + state.home.stats.defensiveRebounds} away={state.away.stats.offensiveRebounds + state.away.stats.defensiveRebounds} /><LiveStat label="Assists" home={state.home.stats.assists} away={state.away.stats.assists} /><LiveStat label="Turnovers" home={state.home.stats.turnovers} away={state.away.stats.turnovers} /><LiveStat label="Fouls" home={state.home.stats.fouls} away={state.away.stats.fouls} /></div></section>}
